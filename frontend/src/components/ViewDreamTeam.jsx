...
const ViewDreamTeam = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await axios.get(`/api/dreamteam/username/${username}`);
      setTeam(res.data);
            ...
            <div className="p-4 space-y-3">{team.players.slice(0, 5).map((player, index) => (
                <div key={player._id} className="flex items-center justify-between bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition group">
                  <div className="flex items-center gap-4">
                    ...
                    <img src={getPlayerImage(player)} alt={player.name} className="h-12 w-12 object-cover rounded-full border-2 border-green-300 group-hover:border-green-500 transition" />
                    <div>
                ...
                {team.players.slice(5, 10).map((player, index) => (
                  <div key={player._id} className="flex items-center justify-between bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition group">
                    <div className="flex items-center gap-4">
                      ...
                      <img src={getPlayerImage(player)} alt={player.name} className="h-12 w-12 object-cover rounded-full border-2 border-yellow-300 group-hover:border-yellow-500 transition" />
                      <div>
                        <Link to={`/players/${player._id}`} className="text-lg font-semibold text-blue-700 hover:underline group-hover:text-blue-900 transition">
                          {player.name}
                        </Link>

...
function FavoriteTeamsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchFavoriteTeams() {
      try {
        ...
        const response = await axios.get('/api/favorites/teams', {
  ...
  const removeFromFavorites = async (teamId) => {
    ...
    try {
      ...
      const currentFavoritesResponse = await axios.get('/api/favorites/teams', {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      await axios.put('/api/favorites/teams', 
        { favoriteTeams: updatedFavoriteIds }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
